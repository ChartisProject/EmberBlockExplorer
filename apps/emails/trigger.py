...
def send_and_log(subject, body_template, to_user=None, to_email=None,
    ...
    if not from_email:
        from_name, from_email = split_email_header(settings.POSTMARK_SENDER)

    body_context_modified = body_context.copy()
    body_context_modified['BASE_URL'] = settings.BASE_URL
    ...
    if settings.EMAIL_DEV_PREFIX:
        send_dict['subject'] += ' [DEV]'
    else:
        # send_dict['bcc_info'] = ','.join([settings.POSTMARK_SENDER, ])
        pass
...
def send_admin_email(subject, body_template, body_context):
    ...
    pm_dict = {
            ...
            'to': ','.join([x[1] for x in settings.ADMINS]),
            'subject': subject,
            'html_body': html_body,
            }
